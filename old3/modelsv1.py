...
class CNN1D(nn.Module):
  def __init__(self,
               seq_len,  # turns out seq_len is not needed
    ...
    self.dnn2 = nn.Linear(in_features=998, out_features=dnn_size)
    self.act2 = nn.ReLU()
  ...
  def forward(self, x):
    ...
    h = self.conv1d(xf)
    # h = self.conv1d(x) # todo: maybe fix it later
...
class CNN1DTransposedRsTDFLstm(nn.Module):
  def __init__(self, seq_len, in_channel_num_of_nucleotides=4, kernel_size_k_mer_motif=8, dnn_size=1024, num_filters=1,
    ...
    self.pooling2 = ReduceSumLambdaLayer(
      m_dim=1)  # nn.MaxPool1d(kernel_size=pooling_kernel_stride, stride=pooling_kernel_stride) # batch, feat, seq
    ...
    # lstm
    lstm_input_size = int(
      seq_len / 4)  # todo why 500? # output_of_pooling2  # 16. seq_len 64 => lstn_is = 16. 2000 -> 500, 8000 -> 2000. therefore 4N => N
  ...
  def forward(self, x):
    timber.debug(f"0 x: {x.shape}")

    xf = x[0]
    xb = x[1]
    h = self.conv1d0(xf) # todo: fix it later
